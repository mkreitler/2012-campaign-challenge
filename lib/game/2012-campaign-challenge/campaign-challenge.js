...
.defines(function(){
  ...
  ig.CampaignChallenge = ig.Game.extend({
    ...
    chooseParty: function(bBackward) {
      this.playSoundTransOpen();

      if (this.curState) {
        this.curState.setTransitionDirection(bBackward ? ig.CCConstants.TRANSDIR.BACKWARD : ig.CCConstants.TRANSDIR.FORWARD);
      }

      if (this.choosePartyState) {
        this.choosePartyState.setTransitionDirection(bBackward ? ig.CCConstants.TRANSDIR.BACKWARD : ig.CCConstants.TRANSDIR.FORWARD);
      }

      this.enterNewState(this.choosePartyState);
    },

    meetOpponents: function(bBackward) {
      this.playSoundTransOpen();
      ...
      if (this.meetOpponentsState) {
        this.meetOpponentsState.setTransitionDirection(bBackward ? ig.CCConstants.TRANSDIR.BACKWARD : ig.CCConstants.TRANSDIR.FORWARD);
      }
