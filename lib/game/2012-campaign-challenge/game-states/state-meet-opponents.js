...
.defines(function(){
  ...
  ig.StateMeetOpponents = ig.BaseState.extend({
    ...
    createLabels: function() {
      this.labelNextPos.x = ig.CCConstants.WIDTH * 0.9;
      this.labelNextPos.y = ig.CCConstants.HEIGHT - ig.game.fontPoster40.height;

      this.labelBackPos.x = ig.CCConstants.WIDTH * 0.1;
    ...
    labelPressed: function(whichLabel) {
      if (whichLabel === this.labelBack) {
        ...
        ig.game.startEvents(ig.CCConstants.EVENT_NAME_FUNDRAISING, ig.CCConstants.EVENT_DESC_FUNDRAISING, false);
      }
      else {
    ...
    clearUI: function() {
      SoftLabel.manager.removeLabel(this.labelBack);
      SoftLabel.manager.removeLabel(this.labelNext);
    ...
    draw: function() {
      ...
      if (this.stateManager) {
        ...
        for (var i=0; i<ig.CCConstants.TEXT_MEET_LINES.length; ++i) {
          ig.game.fontPoster40b.draw(ig.CCConstants.TEXT_MEET_LINES[i], textX, textY + i * ig.game.fontPoster40.height, ig.Font.ALIGN.CENTER);
        }
        ...
        if (ig.game.iconDemocrat && ig.game.iconIndependent && ig.game.iconRepublican) {
          ...
          for (i=0; i<this.images.length; ++i) {
            if (ig.game.getParty() !== i) {
              this.images[i].draw(textX + offsetX, ig.CCConstants.HEIGHT - 1.25 * this.images[i].height);

              var bioTextY = ig.CCConstants.HEIGHT - 1.25 * this.images[i].height - ig.game.fontPhenix40.height * 0.875;
...
//                  font = text.charAt(0) === 'r' ? ig.game.fontPhenix40 : ig.game.fontPhenix40b;
                ...
                font.draw(text.substring(1), textX + offsetX + this.images[0].width * 0.5, bioTextY, ig.Font.ALIGN.CENTER);
